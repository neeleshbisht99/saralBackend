...
from .serializers import (
    ...
    Invoice, InvoiceSerializer, InventoryWithSumSerializer, ShopWithAmountSerializer
)
...
class InventoryGroupView(ModelViewSet):
    queryset = InventoryGroup.objects\
...
class SalePerformanceView(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        query_date = self.request.query_params.dict()
        total = query_date.get('total', None)
        query = self.queryset

        if not total:
            start_date = query_date.get('start_date', None)
            end_date = query_date.get('end_date', None)
        ...
        items = query.annotate(
            sum_of_item=Coalesce(
                Sum("inventory_invoices_quantity"), 0
            )
...
class SaleByShopView(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        query_date = self.request.query_params.dict()
        total = query_date.get('total', None)
        monthly = query_date.get('monthly', None)
        ...
        if monthly:
            shops = query.annotate(month=TruncMonth('created_at'))\
                .values("month", "name").annotate(amount_total=Sum(
